...
type User struct {
	Name        string       `json:"name,omitempty,wildcard"` // string
	Email       *string      `json:"email_ptr,omitempty"`     // pointer
	MyGender    Gender       `json:"gender,omitempty"`        // type alias
	Github      GithubPage   `json:"github,dive,omitempty"`   // struct dive
	NoDive      StructNoDive `json:"no_dive,omitempty"`       // no dive struct
	MyProfile   Profile      `json:"my_profile,omitempty"`    // struct implements its own method
	Arr         []int        `json:"arr,omitempty"`           // normal slice
	MyArr       MySlice      `json:"my_arr,omitempty"`        // slice implements its own method
	IgnoreFiled string       `json:"-"`
}
...
func TestStruct2Map(t *testing.T) {
	...
	res, err := Struct2Map(&user)
	if err != nil {
		t.Errorf("struct to map:%s", err.Error())
		return
	}
	fmt.Println(res["no_dive"])
	for k, v := range res {
		t.Logf("k:%v,v:%v", k, v)
	}
}
